...
public class PlayerSettings implements MySqlData<PlayerSettings> {
    ...
    public static void loadPlayer(@NotNull final UUID joinedPlayer) {
        PlayerSettings settings = new PlayerSettingDatabaseHandler(FileAccess.getSql(),
    ...
    public static void PlayerLeave(UUID leftPlayer) {
        AsyncTask.run(() -> {
            FileAccess.write("Players", leftPlayer.toString(), SETTINGS.get(leftPlayer));
        ...
        MainIF.getIF().getSaveEvents().add(new ConfigSaveEvent() {
            ...
            public Result SaveDataAccess() {
                for (UUID player : SETTINGS.keySet()) {
                    if (!FileAccess.write("Players", player.toString(), SETTINGS.get(player)))
    ...
    @Override
    public PlayerSettings read(@NotNull MySqlDatabase sql) {
        return null;
    }
